...
KnoBL_FML = nan(60,3);
for trNum=1:60
    if ismember(trNum,[1:30])
        KnoBL_FML(trNum,:) = (rotmBL1*K_FML(trNum,:)')';
    else
        KnoBL_FML(trNum,:) = (rotmBL2*K_FML(trNum,:)')';
    end
end
...
axes('Position',[0.08,0.5,0.35,0.35]);
plotSphFrameSegment;
R = axang2rotm([[0 1 0] deg2rad(90)]);
for trNum=1:size(KnoBL,1)
    KnoBL(trNum,:) = (R*KnoBL(trNum,:)')';
end
for trNum=1:size(KnoBL_FML,1)
    KnoBL_FML(trNum,:) = (R*KnoBL_FML(trNum,:)')';
end

desBL1 = R*desBL1';desBL2 = R*desBL2';desTRN = R*desTRN;
desEarlyTFR = axang2rotm([0 0 1 deg2rad(60)])*desBL2;

sFML1 = scatter3(KnoBL_FML(1:30,1),KnoBL_FML(1:30,2),KnoBL_FML(1:30,3),12,'filled','MarkerFaceColor',cBL1,'MarkerEdgeColor','k','LineWidth',0.5);
sFML2 = scatter3(KnoBL_FML(31:60,1),KnoBL_FML(31:60,2),KnoBL_FML(31:60,3),12,'filled','MarkerFaceColor',cBL2,'MarkerEdgeColor','k','LineWidth',0.5);
...
% Curve
ax = axes('Position',[0.6,0.5,0.35,0.3]);
...
p = scatter(-61:-32,err_FML(1:30),12,'filled');
p.MarkerEdgeColor = 'k';
p.MarkerEdgeAlpha = 0.5;
p.MarkerFaceColor = colors(1,:);

p = scatter(-30:-1,err_FML(31:60),12,'filled');
p.MarkerEdgeColor = 'k';
p.MarkerEdgeAlpha = 0.5;
...
p.MarkerFaceColor = colors(1,:);
p = scatter(62:121,err(61:120),12,'filled');
p.MarkerEdgeColor = colors(61,:);
p.MarkerFaceColor = colors(61,:);
p = scatter(123:301,err(121:299),12,'filled');
p.MarkerEdgeColor = colors(121,:);
p.MarkerFaceColor = colors(121,:);
p = scatter(305:363,err(302:360),12,'filled');
p.MarkerEdgeColor = colors(301,:);
p.MarkerFaceColor = colors(301,:);

p = scatter(302,err(300),50,'filled','s');
p.MarkerEdgeColor = 'k';
p.MarkerFaceColor = 'k';
p = scatter(304,err(301),50,'filled','<');
p.MarkerEdgeColor = 'k';
p.MarkerFaceColor = 'k';
...
p = line([303,363],[320,320],'Color','k','LineWidth',1);
 ...
 text(-45,59,'FML1','FontSize',12,'rotation',90)
        text(-15,59,'FML2','FontSize',12,'rotation',90)
        text(17,85,'BL1','FontSize',12)
        text(77,85,'BL2','FontSize',12)
        text(190,85,'TRN','FontSize',12)
        text(320,85,'TFR','FontSize',12)

hold

% Late TRN and Early TFR spheres
innerPos(1,:) = [0.08,0.05,0.35,0.35];%[0.00,0.05,0.55,0.35];
innerPos(2,:) = [0.6,0.05,0.35,0.35];
...
ax = axes('Position',[0.05,0.4,0.02,0.03]);
text(0,0,'(c)','FontName','Times','FontSize',15);
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off'; 
ax = axes('Position',[0.54,0.85,0.02,0.03]);
text(0,0,'(b)','FontName','Times','FontSize',15);
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off'; 
ax = axes('Position',[0.54,0.4,0.02,0.03]);
text(0,0,'(d)','FontName','Times','FontSize',15);
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off'; 

ax = axes('Position',[0.20,0.8,0.02,0.03]);
text(0.5,0.5,'Aiming Axes','interpreter','latex','FontName','Times','FontSize',15)
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off';

ax = axes('Position',[0.22,0.35,0.02,0.03]);
text(0.5,0.5,'Last TRN','FontName','Times','FontSize',15)
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off';

ax = axes('Position',[0.75,0.35,0.02,0.03]);
text(0.5,0.5,'First TFR','FontName','Times','FontSize',15)
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off';

ax = axes('Position',[0.45,0.64,0.07,0.17]);
hold on
s1 = scatter(nan,nan,'filled','MarkerFaceColor',cBL1,'MarkerEdgeColor','k');
s1 = scatter(nan,nan,'filled','MarkerFaceColor',cBL2,'MarkerEdgeColor','k');
...
legend('FML1','FML2','BL1','BL2','TRN','TFR','trial 360','trial 361','FontName','Times','FontSize',15,'position',[0 0 1 1],'box','off')
ax.YAxis.Visible = 'off';
ax.XAxis.Visible = 'off';
hold

% exportgraphics(gcf,'exp2_example.pdf','Resolution',300,'ContentType','vector','BackgroundColor','none')
end